...
区块链技术的发展路线，从可信账簿（BTC）→ 可信代码平台（ETH/EOS）→ 可信执⾏环境（Mixin），未来还有可能会发展到可信硬件。

⽐特币则是世界上第⼀个区块链技术的应⽤，它的核⼼是⼀个公开透明的、不可篡改、分布式可信账簿（BTC）。我就把比特币理解成BTC好了。

以太坊和 EOS 的⽬标都是成为⼀个区块链应⽤平台。

把程序代码放在区块链上，就有了可信代码。

Mixin Network 通过结合 TEE（可信执⾏环境）和 DAG（有向⽆环图）创建了新型的公开透明、不可篡改的分布式数字资产储值⽹络，也就有了可信执⾏环境。

EOS 是⾯向区块链企业的平台。

Mixin Network 的第⼀个 dApp，MixinMessenger，是⾯向个体的平台 —— 除了即时聊天之外，它的᯿要组成部分之⼀就是⽬前⾏业⾥对新⽤户最为友好最为⽅便的分布式多币种钱包（这是底层 Mixin Network 公链的功能）。

李笑来选择的定投组合BOX就包括BTC、EOS、XIN。也就是可信账簿、可信代码平台、可信执行环境。

所以总结来说：全球-区域-行业-若干标的组合，即：全球-中美-区块链-BOX组合
